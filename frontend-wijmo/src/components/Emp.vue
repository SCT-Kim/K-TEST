<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            Emp # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            Emp
        </v-card-title >
        ...
        <v-card-text>
            ...
            <DeptId offline label="DeptId" v-model="value.deptId" :editMode="editMode" @change="change"/>
            <EmpUserManager offline label="EmpUser" v-model="value.empUser" :editMode="editMode" @change="change"/>
        </v-card-text>
...
import EmpBase from '../components/EmpBase.vue'
...
export default {
    ...
    created(){
        ...
        websocketUrl.pathname = "/rsocket/emps";
        websocketUrl.hash = "";
        ...
        client.connect().subscribe({
            ...
            const endpoint = "emps."+ me.value.id +".get"
    ...
    methods: {
        ...
        async save(){
            try {
                ...
                if(!this.offline) {
                    if(this.isNew) {
                        temp = await axios.post(axios.fixUrl('/emps'), this.value)
                    } else {
